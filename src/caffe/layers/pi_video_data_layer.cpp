...
PiVideoDataLayer<Dtype>:: ~PiVideoDataLayer<Dtype>(){
	this->JoinPrefetchThread();
}

template <typename Dtype>
void PiVideoDataLayer<Dtype>:: DataLayerSetUp(const vector<Blob<Dtype>*>& bottom, const vector<Blob<Dtype>*>& top){
	...
	if (this->layer_param_.pi_video_data_param().name_pattern() == ""){
		...
		name_pattern_ = this->layer_param_.pi_video_data_param().name_pattern();
	}
	...
	if (this->layer_param_.pi_video_data_param().modality() == PiVideoDataParameter_Modality_FLOW)
		...
		CHECK(PiReadSegmentRGBToDatum(lines_[lines_id_].first, lines_[lines_id_].second,
									offsets, new_height, new_width, new_length, &datum, true, name_pattern_.c_str()));
	const int crop_size = this->layer_param_.transform_param().crop_size();
	const int batch_size = this->layer_param_.pi_video_data_param().batch_size();
	if (crop_size > 0){
...
template <typename Dtype>
void PiVideoDataLayer<Dtype>::ShuffleVideos(){
...
template <typename Dtype>
void PiVideoDataLayer<Dtype>::InternalThreadEntry(){

	Datum datum;
	CHECK(this->prefetch_data_.count());
	Dtype* top_data = this->prefetch_data_.mutable_cpu_data();
	Dtype* top_label = this->prefetch_label_.mutable_cpu_data();
	PiVideoDataParameter pi_video_data_param = this->layer_param_.pi_video_data_param();
	const int batch_size = pi_video_data_param.batch_size();
	...
	for (int item_id = 0; item_id < batch_size; ++item_id){
		...
		if (this->layer_param_.pi_video_data_param().modality() == PiVideoDataParameter_Modality_FLOW){
			...
			if(!PiReadSegmentRGBToDatum(lines_[lines_id_].first, lines_[lines_id_].second,
									  offsets, new_height, new_width, new_length, &datum, true, name_pattern_.c_str())) {
				continue;
			}
		}
...
INSTANTIATE_CLASS(PiVideoDataLayer);
REGISTER_LAYER_CLASS(PiVideoData);
}
